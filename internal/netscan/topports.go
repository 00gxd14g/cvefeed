package netscan

import (
	"bufio"
	"os"
	"sort"
	"strconv"
	"strings"
	"sync"
)

// nmap ships nmap-services, which carries a measured open-frequency for every
// port from its own internet-wide scans. That data is why "top ports" is worth
// anything: a hand-written list is a list of the ports its author remembers.
//
// The one this replaced found 4 open ports on a real host where a full scan
// found 23, and every service it missed was on a high port nobody would think
// to list — Apache httpd on 7011, 8011 and 30008, RabbitMQ on 30003, Syncthing
// on 8384.
//
// The file is preferred when it is installed, because it is newer than
// anything compiled in here and covers every port rather than a thousand. When
// it is not, the ranking embedded below is used instead. /etc/services is no
// longer consulted: it carries no frequencies, and a list of 218 ports in
// numeric order is narrower than the embedded ranking and pretends to an order
// it does not have.
var servicesFiles = []string{
	"/usr/share/nmap/nmap-services",
	"/usr/local/share/nmap/nmap-services",
	"/opt/homebrew/share/nmap/nmap-services",
}

var (
	topOnce   sync.Once
	topRanked []int
	// topSource says where the ranking came from, for a report that wants to
	// say what "most common" was measured by.
	topSource string
)

// embeddedSource is what topSource reads when no nmap-services file was found.
const embeddedSource = "built-in ranking derived from nmap-services"

// TopPorts returns the n most commonly open TCP ports, most likely first by
// frequency and in ascending numeric order for the scan itself — scanning in
// frequency order gains nothing and makes the output harder to read.
//
// The ranking is nmap's own frequency data: read from nmap-services where nmap
// is installed, and otherwise the thousand-port excerpt of the same data that
// is compiled in. A caller asking for more than the excerpt holds on a machine
// without the file gets the excerpt, which is the honest answer — there is no
// measured order for the rest.
func TopPorts(n int) []int {
	topOnce.Do(loadServices)
	src := topRanked
	if n > len(src) {
		n = len(src)
	}
	out := make([]int, n)
	copy(out, src[:n])
	sort.Ints(out)
	return out
}

// HasPortFrequencies reports whether the ranking in use was measured — read
// from an installed nmap-services file, or the excerpt of that file compiled
// in — so a caller can say "most commonly open" and mean it. It is false only
// when the embedded table is missing, which is a build defect and not a
// property of the host.
func HasPortFrequencies() bool {
	topOnce.Do(loadServices)
	return len(topRanked) > 0
}

// PortRankingSource names where the ranking came from, for a report.
func PortRankingSource() string {
	topOnce.Do(loadServices)
	return topSource
}

func loadServices() {
	topRanked, topSource = loadRanking(servicesFiles)
}

// loadRanking reads the first nmap-services file that exists and falls back to
// the embedded ranking. It is separate from loadServices so a test can ask what
// a machine without nmap gets without depending on what this machine has.
func loadRanking(files []string) ([]int, string) {
	for _, path := range files {
		if ranked := parseServices(path); len(ranked) > 0 {
			return ranked, "nmap-services at " + path
		}
	}
	return append([]int(nil), topPortsEmbedded...), embeddedSource
}

type portFreq struct {
	port int
	freq float64
}

// parseServices reads "name port/proto frequency" lines, keeping TCP.
func parseServices(path string) []int {
	f, err := os.Open(path)
	if err != nil {
		return nil
	}
	defer f.Close()

	seen := map[int]float64{}
	sc := bufio.NewScanner(f)
	for sc.Scan() {
		line := sc.Text()
		if i := strings.IndexByte(line, '#'); i >= 0 {
			line = line[:i]
		}
		fields := strings.Fields(line)
		if len(fields) < 2 {
			continue
		}
		portProto, ok := strings.CutSuffix(fields[1], "/tcp")
		if !ok {
			continue
		}
		port, err := strconv.Atoi(portProto)
		if err != nil || port < 1 || port > 65535 {
			continue
		}
		freq := 0.0
		if len(fields) >= 3 {
			freq, _ = strconv.ParseFloat(fields[2], 64)
		}
		if f, dup := seen[port]; !dup || freq > f {
			seen[port] = freq
		}
	}
	if sc.Err() != nil || len(seen) == 0 {
		return nil
	}

	list := make([]portFreq, 0, len(seen))
	for p, f := range seen {
		list = append(list, portFreq{p, f})
	}
	// Ties broken by port number so the ranking is the same on every machine,
	// which matters because it decides what a default scan looks at.
	sort.Slice(list, func(i, j int) bool {
		if list[i].freq != list[j].freq {
			return list[i].freq > list[j].freq
		}
		return list[i].port < list[j].port
	})
	out := make([]int, len(list))
	for i, pf := range list {
		out[i] = pf.port
	}
	return out
}

// topPortsEmbedded is the thousand most commonly open TCP ports, most common
// first, for a machine that has no nmap-services file to read.
//
// The numbers were derived from nmap's nmap-services frequency data (Nmap
// Project, https://nmap.org/, distributed under the Nmap Public Source
// License) as shipped with nmap 7.94 (file dated 2024-04-01), on 2026-09-01:
// the TCP entries sorted by open-frequency descending, ties by port ascending,
// first thousand. Only the port numbers are kept; the frequencies themselves
// are not needed once the order is fixed. It is the same excerpt nmap itself
// scans by default, which is why a default scan here and "nmap" with no -p
// look at the same ports.
//
// Regenerate by reading a newer nmap-services the same way; the ranking
// changes slowly, and an old excerpt is still a measured one.
var topPortsEmbedded = []int{
	80, 23, 443, 21, 22, 25, 3389, 110, 445, 139, 143, 53, 135, 3306, 8080, 1723, 111, 995, 993, 5900,
	1025, 587, 8888, 199, 1720, 465, 548, 113, 81, 6001, 10000, 514, 5060, 179, 1026, 2000, 8443, 8000, 32768, 554,
	26, 1433, 49152, 2001, 515, 8008, 49154, 1027, 5666, 646, 5000, 5631, 631, 49153, 8081, 2049, 88, 79, 5800, 106,
	2121, 1110, 49155, 6000, 513, 990, 5357, 427, 49156, 543, 544, 5101, 144, 7, 389, 8009, 3128, 444, 9999, 5009,
	7070, 5190, 3000, 5432, 1900, 3986, 13, 1029, 9, 5051, 6646, 49157, 1028, 873, 1755, 2717, 4899, 9100, 119, 37,
	1000, 3001, 5001, 82, 10010, 1030, 9090, 2107, 1024, 2103, 6004, 1801, 5050, 19, 8031, 1041, 255, 1048, 1049, 1053,
	1054, 1056, 1064, 1065, 2967, 3703, 17, 808, 3689, 1031, 1044, 1071, 5901, 100, 9102, 1039, 2869, 4001, 5120, 8010,
	9000, 2105, 636, 1038, 2601, 1, 7000, 1066, 1069, 625, 311, 280, 254, 4000, 1761, 5003, 2002, 1998, 2005, 1032,
	1050, 6112, 3690, 1521, 2161, 1080, 6002, 2401, 902, 4045, 787, 7937, 1058, 2383, 32771, 1033, 1040, 1059, 50000, 5555,
	10001, 1494, 3, 593, 2301, 3268, 7938, 1022, 1234, 1035, 1036, 1037, 1074, 8002, 9001, 464, 497, 1935, 2003, 6666,
	6543, 24, 1352, 3269, 1111, 407, 500, 20, 2006, 1034, 1218, 3260, 15000, 4444, 264, 33, 2004, 1042, 42510, 999,
	3052, 1023, 222, 1068, 888, 7100, 563, 1717, 992, 2008, 32770, 7001, 32772, 2007, 8082, 5550, 512, 1043, 2009, 5801,
	1700, 2701, 7019, 50001, 4662, 2065, 42, 2010, 161, 2602, 3333, 9535, 5100, 2604, 4002, 5002, 1047, 1051, 1052, 1055,
	1060, 1062, 1311, 2702, 3283, 4443, 5225, 5226, 6059, 6789, 8089, 8192, 8193, 8194, 8651, 8652, 8701, 9415, 9593, 9594,
	9595, 16992, 16993, 20828, 23502, 32769, 33354, 35500, 52869, 55555, 55600, 64623, 64680, 65000, 65389, 1067, 13782, 366, 5902, 9050,
	85, 1002, 5500, 1863, 1864, 5431, 8085, 10243, 45100, 49999, 51103, 49, 90, 6667, 1503, 6881, 27000, 340, 1500, 8021,
	2222, 5566, 8088, 8899, 9071, 1501, 5102, 6005, 9101, 9876, 32773, 32774, 163, 5679, 146, 648, 1666, 901, 83, 3476,
	5004, 5214, 8001, 8083, 8084, 9207, 14238, 30, 912, 12345, 2030, 2605, 6, 541, 4, 1248, 3005, 8007, 306, 880,
	2500, 1086, 1088, 1097, 2525, 4242, 8291, 9009, 52822, 900, 6101, 2809, 7200, 211, 800, 987, 1083, 12000, 32775, 705,
	711, 20005, 6969, 13783, 1045, 1046, 1057, 1061, 1063, 1070, 1072, 1073, 1075, 1077, 1078, 1079, 1081, 1082, 1085, 1093,
	1094, 1096, 1098, 1099, 1100, 1104, 1106, 1107, 1108, 1148, 1169, 1272, 1310, 1687, 1718, 1783, 1840, 1947, 2100, 2119,
	2135, 2144, 2160, 2190, 2260, 2381, 2399, 2492, 2607, 2718, 2811, 2875, 3017, 3031, 3071, 3211, 3300, 3301, 3323, 3325,
	3351, 3367, 3404, 3551, 3580, 3659, 3766, 3784, 3801, 3827, 3998, 4003, 4126, 4129, 4449, 5030, 5222, 5269, 5414, 5633,
	5718, 5810, 5825, 5877, 5910, 5911, 5925, 5959, 5960, 5961, 5962, 5987, 5988, 5989, 6123, 6129, 6156, 6389, 6580, 6788,
	6901, 7106, 7625, 7627, 7741, 7777, 7778, 7911, 8086, 8087, 8181, 8222, 8333, 8400, 8402, 8600, 8649, 8873, 8994, 9002,
	9010, 9011, 9080, 9220, 9290, 9485, 9500, 9502, 9503, 9618, 9900, 9968, 10002, 10012, 10024, 10025, 10566, 10616, 10617, 10621,
	10626, 10628, 10629, 11110, 11967, 13456, 14000, 14442, 15002, 15003, 15660, 16001, 16016, 16018, 17988, 19101, 19801, 19842, 20000, 20031,
	20221, 20222, 21571, 22939, 24800, 25734, 27715, 28201, 30000, 30718, 31038, 32781, 32782, 33899, 34571, 34572, 34573, 40193, 48080, 49158,
	49159, 49160, 50003, 50006, 50800, 57294, 58080, 60020, 63331, 65129, 89, 691, 212, 1001, 1999, 2020, 32776, 2998, 6003, 7002,
	50002, 32, 898, 2033, 3372, 5510, 99, 425, 749, 5903, 43, 458, 5405, 6106, 6502, 7007, 13722, 1087, 1089, 1124,
	1152, 1183, 1186, 1247, 1296, 1334, 1580, 1782, 2126, 2179, 2191, 2251, 2522, 3011, 3030, 3077, 3261, 3369, 3370, 3371,
	3493, 3546, 3737, 3828, 3851, 3871, 3880, 3918, 3995, 4006, 4111, 4446, 5054, 5200, 5280, 5298, 5822, 5859, 5904, 5915,
	5922, 5963, 7103, 7402, 7435, 7443, 7512, 8011, 8090, 8100, 8180, 8254, 8500, 8654, 9091, 9110, 9666, 9877, 9943, 9944,
	9998, 10004, 10778, 15742, 16012, 18988, 19283, 19315, 19780, 24444, 27352, 27353, 27355, 32784, 49163, 49165, 49175, 50389, 50636, 51493,
	55055, 56738, 61532, 61900, 62078, 1021, 9040, 32777, 32779, 616, 666, 700, 2021, 32778, 84, 545, 1112, 1524, 2040, 4321,
	5802, 38292, 49400, 1084, 1600, 2048, 2111, 3006, 32780, 2638, 6547, 6699, 9111, 16080, 555, 667, 720, 801, 1443, 1533,
	2034, 2106, 5560, 6007, 1090, 1091, 1114, 1117, 1119, 1122, 1131, 1138, 1151, 1175, 1199, 1201, 1271, 1862, 2323, 2393,
	2394, 2608, 2725, 2909, 3003, 3168, 3221, 3322, 3324, 3390, 3517, 3527, 3800, 3809, 3814, 3826, 3869, 3878, 3889, 3905,
	3914, 3920, 3945, 3971, 4004, 4005, 4279, 4445, 4550, 4567, 4848, 4900, 5033, 5061, 5080, 5087, 5221, 5440, 5544, 5678,
	5730, 5811, 5815, 5850, 5862, 5906, 5907, 5950, 5952, 6025, 6100, 6510, 6565, 6566, 6567, 6689, 6692, 6779, 6792, 6839,
	7025, 7496, 7676, 7800, 7920, 7921, 7999, 8022, 8042, 8045, 8093, 8099, 8200, 8290, 8292, 8300, 8383, 8800, 9003, 9081,
	9099, 9200, 9418, 9575, 9878, 9898, 9917, 10003, 10009, 10180, 10215, 11111, 12174, 12265, 14441, 15004, 16000, 16113, 17877, 18040,
	18101, 19350, 25735, 26214, 27356, 30951, 32783, 32785, 40911, 41511, 44176, 44501, 49161, 49167, 49176, 50300, 50500, 52673, 52848, 54045,
	54328, 55056, 56737, 57797, 60443, 70, 417, 617, 714, 722, 777, 981, 1009, 2022, 4224, 4998, 6346, 301, 524, 668,
	765, 1076, 2041, 5999, 10082, 259, 416, 1007, 1417, 1434, 1984, 2038, 2068, 4343, 6009, 7004, 44443, 109, 687, 726,
	911, 1010, 1461, 2035, 2046, 4125, 6006, 7201, 9103, 125, 481, 683, 903, 1011, 1455, 2013, 2043, 2047, 6668, 6669,
	256, 406, 783, 843, 2042, 2045, 5998, 9929, 31337, 44442, 1092, 1095, 1102, 1105, 1113, 1121, 1123, 1126, 1130, 1132,
	1137, 1141, 1145, 1147, 1149, 1154, 1163, 1164, 1165, 1166, 1174, 1185, 1187, 1192, 1198, 1213, 1216, 1217, 1233, 1236,
	1244, 1259, 1277, 1287, 1300, 1301, 1309, 1322, 1328, 1556, 1583, 1594, 1641, 1658, 1688, 1719, 1721, 1805, 1812, 1839,
	1875, 1914, 1971, 1972, 1974, 2099, 2170, 2196, 2200, 2288, 2366, 2382, 2557, 2710, 2800, 2910, 2920, 2968, 3007, 3013,
}
